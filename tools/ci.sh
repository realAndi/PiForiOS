...
#
# make-repo.py and fetch-published.sh are not in this repository any more: they
# are identical in every port, so they live in realAndi/ios-port-ci and a fix to
# one of them reaches all the ports at once.
#
...
# ref, so the README's build steps behave the same on a laptop as in CI rather
# than only ever being exercised by the workflow.
set -euo pipefail
